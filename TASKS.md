...
- [ ] `todo` — **T7**: Asset criticality + priority scoring module (weighted formula version first)
      <br>_**T6 is waiting on this.** `MaintenanceTask.priority` currently carries raw severity 1–5 as a placeholder; every decision-log entry is flagged `priorityIsPlaceholder: true`. Swapping in the real FR2.3 score is a one-line change at the call site in `scripts/real_data.py`._
      <br>_Asset criticality is already computed and seeded (`assets.criticalityScore`, 27.9–90.8, with a `criticalityBreakdown`) — T7 needs to combine it with severity, SLA urgency and (later) T16's risk score._
      <br>_⚠️ Fold **overdue-ness** into priority here, not into feasibility: 17 of 89 tasks are already past SLA, and T6 deliberately keeps SLA soft (D-020)._
...
- [ ] `todo` — **T10**: Express API — auth (JWT), CRUD for tasks/corridors/assets, orchestration calls to FastAPI service
      ...
      <br>_**Still pending:** JWT auth + role gating (FR10), write/CRUD paths for task submission (FR1.1), CSV/JSON bulk import (FR1.3), and the orchestration calls to the optimizer (`/prioritize`, `/optimize`, `/baseline`) which need T9 first._
