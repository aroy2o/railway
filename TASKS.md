...
- [ ] `todo` — **T5**: MongoDB schemas (Mongoose) for all PRD Section 15 collections
      <br>_All six collections now have real data to model: `corridors.json`, `corridor_calendar.json`, `stations.json`, `trains.json`, `assets.json`, `tasks.json`, `resources.json`._
      <br>_⚠️ `corridors.maxDailyBlockWindows` is joined in from `corridor_calendar.json` on `_id` at seed time (D-009) — it is deliberately empty in `corridors.json`._
      <br>_⚠️ Preserve `synthetic: true` and the `fieldProvenance` block through seeding — the honesty framing must survive into the database and the UI (D-015), not stop at the file._
