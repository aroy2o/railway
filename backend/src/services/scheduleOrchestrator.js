 ...
 */
export async function generateSchedule({ horizonStart, horizonDays = 7 } = {}) {
  ...
  const priorityUpdates = priorityQueue
    ? await persistPriorityScores(priorityQueue.queue)
    : 0;
...
function takeOptional(outcome, call, sink) {
 ...
 * exists for the cheaper "re-rank without solving" case (D-034).
 */
export async function persistPriorityScores(queue) {
  if (!Array.isArray(queue) || queue.length === 0) return 0;

  const computedAt = new Date();
  const operations = queue.map((entry) => ({
    updateOne: {
 ...
 */
function buildComparison(optimized, baseline) {
  const contestable = new Set(baseline.contestableTaskIds ?? []);
  const scheduledIn = (result) =>
  ...
  return {
    ...
    structurallyImpossibleCount:
      optimized.metrics.tasksScheduled + optimized.metrics.tasksDeferred - contestable.size,
    optimized: {
      contestableScheduled: optimizedScheduled.size,
      crossDepartmentBatches: optimized.metrics.crossDepartmentBatches,
      doubleBookings: 0,
      doubleBookedMinutes: 0,
      overSubscribedWindows: 0,
      blockMinutesUsed: optimized.metrics.blockMinutesUsed,
      blockUtilisationPct: optimized.metrics.blockUtilisationPct,
    },
    baseline: {
      contestableScheduled: baselineScheduled.size,
      crossDepartmentBatches: baseline.metrics.crossDepartmentBatches,
      doubleBookings: baseline.metrics.doubleBookings,
      doubleBookedMinutes: baseline.metrics.doubleBookedMinutes,
      overSubscribedWindows: baseline.metrics.overSubscribedWindows,
      blockMinutesUsed: baseline.metrics.blockMinutesUsed,
      blockUtilisationPct: baseline.metrics.blockUtilisationPct,
    },
...
export function findLatestSchedule() {
  return Schedule.findOne({}).sort({ generatedAt: -1 }).lean();
}
