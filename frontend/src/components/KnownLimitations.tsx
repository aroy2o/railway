...
export function KnownLimitations({
  ...
  // Collapsed by default: the densest, least time-critical panel in the
  // sidebar, so it doesn't cost permanent vertical space just to be
  ...
  // Collapsed-state summary count: every distinct thing this plan actually
  // flags as a gap - live conflicts (or the pre-T21 fallback counts),
  // entirely-unchecked types, and a partial-generation failure. Deliberately
  // excludes `checkedAndClear` - those are reassurances, not limitations.
  const totalFlags =
    (hasTypedReport ? groups.length : countsOnly.filter((item) => item.count > 0).length) +
    notYetDetectable.length +
    (generationErrors.length > 0 ? 1 : 0)
      ...
      {expanded && (
        <>
          <p className="mt-1 mb-3 text-xs text-slate-500">
            Reported by the solver itself, so the plan is never presented as more constrained than
            it actually is. Each type carries the strategy that <em>would</em> resolve it —
            labelled, not applied.
          </p>
          ...
          {hasTypedReport ? (
            groups.length > 0 ? (
              ...
              <p className="text-xs text-slate-500">
                No live conflicts of any PRD 9.5 type on the optimized plan — see "Checked, and
                none found" below for what that claim is based on.
              </p>
            )
          ) : (
            <ul className="space-y-2">
